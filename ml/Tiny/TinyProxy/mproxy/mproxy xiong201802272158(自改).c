...
int read_header(int fd, void * buffer)
{
	...
	receive_data(fd, line_buffer, MAX_HEADER_SIZE);
	strncpy(base_ptr,line_buffer,strlen(line_buffer)+1); 
...
int extract_host(const char *header) {
    char * _p = strstr(header,"VPN");  /* 在 xiongml 方法中解析 隧道主机名称及端口号 */
	if(_p){
		char * p1 = strchr(_p,' ');
		char * p2 = strchr(p1 + 1,':');
		char * p3 = strchr(p1 + 1,' ');
		if(p2){
			...
			if(p3){
				...
				strncpy(s_port,p2+1,strlen(p2) -1);
			}
...
void get_info(char * output)
{
    ...
    sprintf(line_buffer,"======= mproxy大湿胸免流修改版 (v0.3) ========\n");
